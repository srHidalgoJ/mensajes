    ...
    try:
        ...
        if request.method == 'GET': 
            ...
            attempt = db.execute(
                ...
                ' FROM activationlink'
                ' WHERE challenge=? AND state=?', (number, utils.U_UNCONFIRMED)
            ).fetchone()
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id, userid FROM forgotlink WHERE challenge=? AND state=?', 
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT id from forgotlink WHERE challenge=? AND state=?', 
